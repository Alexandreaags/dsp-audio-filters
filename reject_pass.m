...
fc1 = 250;             % Frequência de corte inferior (Hz)
fc2 = 2000;            % Frequência de corte superior (Hz)

% Frequências de corte normalizadas
wc1 = 2 * pi * (fc1 / Fs);
wc2 = 2 * pi * (fc2 / Fs);

%%%%%%%%%%%%%%% Filtro RF (Rejeita-Faixa) %%%%%%%%%%%%%%
% Equação do filtro rejeita-faixa ideal
h_0 = 1 - (wc2 - wc1) / pi;
h_positivo = zeros(1, M);
for n=1:M
    h_positivo(n) = sin(wc1*n)/(pi*n) - sin(wc2*n)/(pi*n);
end
...
%% Audio filtrado
audiowrite('minha_gravacao_RF.wav', sinal_filtrado, Fs);
disp('Áudio filtrado salvo como "minha_gravacao_RF.wav"');
