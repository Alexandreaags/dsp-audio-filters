...
%% 2. Parâmetros do Filtro Passa-Alta (IIR)
...
fs_stop = 1000; % Frequência de rejeição (1 oitava abaixo)

% Pré-distorção de frequências para Transformada Bilinear
wd_p = fp / (Fs/2) * pi;
...
%% 3. Projeto do Filtro Analógico e Transformada Bilinear
% Cálculo da ordem (Chebyshev Tipo I para garantir o ripple de 1dB)
[M, wc] = cheb1ord(wa_p, wa_s, Rp, Rs, 's');
[sb, sa] = cheby1(M, Rp, wc, 'high', 's');

% Transformada Bilinear para o domínio digital
[zb, za] = bilinear(sb, sa, 1/Fs);

%% 4. Resposta em Frequência
figure;
freqz(zb, za, 10000, Fs);
title('Resposta em Frequência - Passa-Alta (IIR)');
...
subplot(2,1,2);
plot(sinal_filtrado);
title('Sinal Filtrado no Tempo (Passa-Alta IIR)');

audiowrite('minha_gravacao_IIR_PA.wav', sinal_filtrado, Fs);
disp('Áudio filtrado salvo como "minha_gravacao_IIR_PA.wav"');
...
title('Espectros: Original vs Filtrado (Passa-Alta IIR)');
xlabel('Frequência (Hz)');
ylabel('Magnitude');
legend('Sinal Original', 'Sinal Filtrado');
xlim([0 5000]); % Foco na transição
hold off;